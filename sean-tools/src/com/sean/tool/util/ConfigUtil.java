 ...
 * <p/>
 * An ConfigUtil instance is a map of {@link ConfigUtil.Section Section}s, keyed by section name.  Each
 * {@code Section} is itself a map of {@code String} name/value pairs.  Name/value pairs are guaranteed to be unique
 * within each {@code Section} only - not across the entire {@code ConfigUtil} instance.
 *
 * @since 1.0
 */
public class ConfigUtil implements Map<String, ConfigUtil.Section> {

    private static transient final Logger log = Logger.getLogger(ConfigUtil.class);
    ...
    /**
     * Creates a new empty {@code ConfigUtil} instance.
     */
    public ConfigUtil() {
    ...
    /**
     * Creates a new {@code ConfigUtil} instance with the specified defaults.
     *
     * @param defaults the default sections and/or key-value pairs to copy into the new instance.
     */
    public ConfigUtil(ConfigUtil defaults) {
        this();
        if (defaults == null) {
    ...
    /**
     * Returns the names of all sections managed by this {@code ConfigUtil} instance or an empty collection if there are
     * no sections.
     *
     * @return the names of all sections managed by this {@code ConfigUtil} instance or an empty collection if there are
     *         no sections.
     */
    ...
    /**
     * Returns the sections managed by this {@code ConfigUtil} instance or an empty collection if there are
     * no sections.
     *
     * @return the sections managed by this {@code ConfigUtil} instance or an empty collection if there are
     *         no sections.
     */
    ...
    /**
     * Creates a new {@code ConfigUtil} instance loaded with the INI-formatted data in the resource at the given path.  The
     ...
     *
     * @param resourcePath the resource location of the INI data to load when creating the {@code ConfigUtil} instance.
     * @return a new {@code ConfigUtil} instance loaded with the INI-formatted data in the resource at the given path.
     * @throws ConfigurationException if the path cannot be loaded into an {@code ConfigUtil} instance.
     */
    public static ConfigUtil fromResourcePath(String resourcePath) throws ConfigurationException {
        if (!StringUtils.hasLength(resourcePath)) {
            throw new IllegalArgumentException("Resource Path argument cannot be null or empty.");
        }
        ConfigUtil ini = new ConfigUtil();
        ini.loadFromPath(resourcePath);
        return ini;
    }

    /**
     * Loads data from the specified resource path into this current {@code ConfigUtil} instance.  The
     * resource path may be any value interpretable by the
     * {@link ResourceUtils#getInputStreamForPath(String) ResourceUtils.getInputStreamForPath} method.
     *
    ...
    public boolean equals(Object obj) {
        if (obj instanceof ConfigUtil) {
            ConfigUtil ini = (ConfigUtil) obj;
    ...
    public String toString() {
        if (CollectionUtils.isEmpty(this.sections)) {
            ...
            int i = 0;
            for (ConfigUtil.Section section : this.sections.values()) {
                if (i > 0) {
                    sb.append(",");
                }
    ...
    /**
     * An {@code ConfigUtil.Section} is String-key-to-String-value Map, identifiable by a
     * {@link #getName() name} unique within an {@link ConfigUtil} instance.
     */
