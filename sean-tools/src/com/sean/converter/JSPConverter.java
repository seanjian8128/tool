 ...
 * 
 */
public class JSPConverter implements AbstractConverter {

	@Override
	public Collection<String> perform(Collection<String> lines)
			...
			for (String line : lines) {
				try {
					...
					tranLine = tranLine.replaceAll("\\b[Gg][Bb]18030\\b", Config
							.getConfig().getConverterTargetCharset());
//					if (tranLine.indexOf("<%@ include") >= 0)
//						System.out.println(tranLine);
//					if (tranLine.indexOf("commonLib.jsp") < 0) {
//						tranLine = tranLine.replaceAll(
//								"<%@\\s{0,}include\\s{1,}file",
//								"<jsp:include page");
//						if (tranLine.indexOf("<jsp:include ") >= 0) {
//							tranLine = tranLine.replaceAll("%>", "/>");
//							System.out.println(tranLine);
//						}
//					}
