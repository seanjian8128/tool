...
public class ConverterTask {
	...
	private void converterFile(File source, File targetDir) {
		...
		String charset = null;
		try {
			charset = FileCharsetDetector.getCharsetDetector().guessFileEncoding(source);
			if(!StringUtils.isNotBlank(charset)){
				charset = "utf8";
			}
			else{
...
////			    System.out.println(file.getName() + "：可能是GBK，也可能是其他编码");  
			List<String> sourceLines = FileUtils.readLines(source, "GBK");
			if (sourceLines != null) {
				...
				try {
					...
					log.error("tranfer file[" + source.getPath()
